...
{
    public partial class SupplierFinder : Form
    {
        ModelSupplier model;
        ...
        {
            IQueryable querySupplier = model.findByName(this.textBox1.Text);
            this.dataGridView1.DataSource = querySupplier;
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (this.dataGridView1.SelectedRows.Count > 0)
            {
                ...
                EntitySupplier supplier = (EntitySupplier)model.find(prkey);
                if (this.Owner.Name == "OrdersEditor")
                {
                    ((OrdersEditor)this.Owner).fillSupplier(supplier);
                    this.Dispose();
                }
